...
function filterFiles() {
  ...
  // Update file count
  fileCount.textContent = `${filteredFiles.length} (filtered from ${selectedFiles.length})`;
  ...
  filteredFiles.forEach((file, index) => {
    ...
    for (const [type, extensions] of Object.entries(fileTypeMap)) {
      if (extensions.includes(extension)) {
        fileTypeLabel = type.charAt(0).toUpperCase() + type.slice(1);
        break;
      }
    }
...
function handleSingleFile(file) {
  ...
  file.arrayBuffer().then(arrayBuffer => {
    ...
    originalHash.textContent = `Original MD5: ${hash}`;
    modifyBtn.disabled = false;
    status.textContent = '';
    status.className = 'status';
  }).catch(error => {
    console.error('Error calculating hash:', error);
    status.textContent = 'Error calculating hash: ' + error.message;
...
async function showFilePreview(file, modifiedArray, changedPosition) {
  ...
  if (fileType === 'image') {
    ...
    try {
      ...
      previewContent.textContent = 'Error loading text preview: ' + error.message;
    }
  } else {
    ...
    if (modifiedArray.length > 256) {
      ...
      ellipsis.textContent = `... ${modifiedArray.length - 256} more bytes`;
  ...
  try {
    ...
    if (appendData.checked) {
      ...
      if (position >= uint8Array.length) {
        throw new Error(`Position ${position} is out of bounds (file size: ${uint8Array.length} bytes)`);
      }
...
modifyBtn.addEventListener('click', async () => {
  ...
  try {
    ...
    if (result.success) {
      ...
      modifiedHash.textContent = `Modified MD5: ${result.newHash}`;
      ...
      status.textContent = appendData.checked 
        ? `Appended "${appendText.value}" to the end of the file. MD5 changed from ${result.originalHash} to ${result.newHash}` 
        : `Changed byte at position ${bytePosition.value} to 0x${byteValue.value}. MD5 changed from ${result.originalHash} to ${result.newHash}`;
    ...
    console.error('Error modifying file:', error);
    status.textContent = 'Error modifying file: ' + error.message;
    status.className = 'status error';
  }
});
...
modifyAllBtn.addEventListener('click', async () => {
  ...
  status.textContent = `Modified ${successCount} files successfully. ${failCount} files failed.`;
